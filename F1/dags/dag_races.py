...
@dag(
    ...
    #schedule= '@weekly',
    schedule='0 12 */2 * *',  # minute hour day-of-month month day-of-week
    catchup= False,
        ...
        for year in seasons_list:
            # if year == 2025:
                ...
                if response.status_code == 200 :
                    ...
                    data = response.json()
                    race = pd.json_normalize(data['races'],sep = '_', record_prefix='race_')
                    ...
                    race['lastmodifieddatetime'] = datetime.now()
                    races[year] = race
                else:
...
#                     # Rename metadata 'time' to avoid conflict
#                 for race in race_data:
#                     if 'time' in race:
#                         race['race_time'] = race.pop('time')
                        
                        
#                     race_results_df = pd.json_normalize(
#                                         data=race_data,
#                                         record_path=['results'],
#                                         meta=['season', 'round', 'date', 'race_time', 'raceId', 'raceName', 'url',
        ...
        for _, row in races_df.iterrows():
            if row['season'] == 2025:
                ...
                if response.status_code == 200:
                    print(f"Successful retrieval for {url}")
                    data = response.json()
                    race_data = data['races']  # single dict

                    # Rename 'time' to 'race_time'
                    if 'time' in race_data:
                        race_data['race_time'] = race_data.pop('time')

                    # Add 'season' to race_data
                    race_data['season'] = data.get('season')
